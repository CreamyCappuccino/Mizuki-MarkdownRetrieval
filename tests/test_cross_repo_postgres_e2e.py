...
if not DATABASE_URL:
    pytest.skip(
        "real Postgres E2E requires MDR_TEST_DATABASE_URL",
...
def test_real_pgvector_provider_refresh_and_related_search(tmp_path) -> None:
    _write_fixture(tmp_path)
    namespace = "cross-repo-pg-e2e"
    state_path = tmp_path / "local" / "index_state.json"
    schema = f"mdr_cross_repo_{uuid.uuid4().hex}"
    scope = ScopeConfig(namespace=namespace, root=tmp_path, recursive=True)
    embedding = KeywordEmbeddingProvider()
    provider_revision = "pgvector-keyword-fixture-v1"
    ...
    try:
        ...
        reopened = retrieval_toolkit.PostgresIndexProvider(
            DATABASE_URL,
            schema=schema,
            vector_dimensions=3,
            embedding_provider=embedding,
